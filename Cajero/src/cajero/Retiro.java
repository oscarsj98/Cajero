...
public class Retiro extends JFrame {
    ...
    private void operacionPulsado(String tecla) {
        ...
        if (tecla.equals("ATRAS")) {
            dispose();
            Inicio c = new Inicio(this.saldo);
        }
        nuevaOperacion = true;
    }
    ...
    private void calcularResultado() {
        if (saldo <= 0) {
            JOptionPane.showMessageDialog(null, "Usted no tiene fondos");

        } else if (resultado > saldo) {
            JOptionPane.showMessageDialog(null, "No cuenta con esta cantidad para retirar");

        } else {
            saldo = (saldo - resultado);
            JOptionPane.showMessageDialog(null, "ha retirado exitosamente, su saldo actual es de: " + saldo);
